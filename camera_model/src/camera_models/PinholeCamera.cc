...
#include "camodocal/gpl/gpl.h"    // "camodocal/gpl/gpl.h"
...
bool PinholeCamera::Parameters::readFromYamlFile(const std::string &filename)
{
  ...
  m_modelType = PINHOLE;                                // 相机类型
  fs["camera_name"] >> m_cameraName;                    // 相机名称
  m_imageWidth = static_cast<int>(fs["image_width"]);   // 图片宽度
  m_imageHeight = static_cast<int>(fs["image_height"]); // 图片高度

  cv::FileNode n = fs["distortion_parameters"];         // 畸变参数: k1, k2, p1, p2
  m_k1 = static_cast<double>(n["k1"]);
  m_k2 = static_cast<double>(n["k2"]);
  m_p1 = static_cast<double>(n["p1"]);
  m_p2 = static_cast<double>(n["p2"]);

  n = fs["projection_parameters"];                      // 相机内参矩阵: fx, fy, cx, cy
...
{
  // 调用函数liftProjective()，从二维像素坐标p得到去畸变的归一化平面坐标P（z=1）
  liftProjective(p, P);
...
void PinholeCamera::liftProjective(const Eigen::Vector2d &p, Eigen::Vector3d &P) const
{
  ...
  // (1) 将二维像素点反投影到归一化平面，得到畸变后的归一化坐标
    // Lift points to normalised plane
  mx_d = m_inv_K11 * p(0) + m_inv_K13;
  my_d = m_inv_K22 * p(1) + m_inv_K23;

  // (2) 利用逆畸变模型，得到畸变前的归一化坐标
  if (m_noDistortion)
  {
    mx_u = mx_d;
    my_u = my_d;
  ...
  P << mx_u, my_u, 1.0; // 去畸变的归一化平面坐标（z=1）
}
