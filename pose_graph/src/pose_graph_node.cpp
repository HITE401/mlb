...
int VISUALIZATION_SHIFT_X;  // 在进行位姿图优化后，对得到的位姿在x坐标和y坐标的偏移量（一般都设为0）
int VISUALIZATION_SHIFT_Y;
int ROW;                    // 图片高度，图片宽度
int COL;
int DEBUG_IMAGE;            // 是否输出位姿图的图片，用于调试: 1
int VISUALIZE_IMU_FORWARD;  // 是否输出IMU前向传播的高频结果: 0
int LOOP_CLOSURE;           // 是否执行闭环: 1-执行; 2-关闭;
int FAST_RELOCALIZATION;    // 是否打开快速重定位: 0

camodocal::CameraPtr m_camera;  // 相机模型（已从yaml文件中读取相机参数）
Eigen::Vector3d tic;
Eigen::Matrix3d qic;
...
{
  if (VISUALIZE_IMU_FORWARD)  // 如果VISUALIZE_IMU_FORWARD = 1
  {
    // 位置和方向四元数
...
int main(int argc, char **argv)
{
  ...
  // 2. 调用位姿图对象方法posegraph.registerPub(n)，发布话题
  posegraph.registerPub(n);

  // 3. 从launch文件中读取参数，再从yaml配置文件中读取参数
    // 调用ROS句柄方法n.getParam()，从launch文件中读取参数
  n.getParam("visualization_shift_x", VISUALIZATION_SHIFT_X); // 在进行位姿图优化后，对得到的位姿在x坐标和y坐标的偏移量（一般都设为0）
  n.getParam("visualization_shift_y", VISUALIZATION_SHIFT_Y);
  n.getParam("skip_cnt", SKIP_CNT);
  n.getParam("skip_dis", SKIP_DIS);
    
    // 打开yaml配置文件
  std::string config_file;  // yaml配置文件路径
  ...
  LOOP_CLOSURE = fsSettings["loop_closure"];  // 是否打开闭环
  std::string IMAGE_TOPIC;
  int LOAD_PREVIOUS_POSE_GRAPH;

    // 如果需要进行回环（LOOP_CLOSURE=1）
  if (LOOP_CLOSURE)
  {
    ROW = fsSettings["image_height"];
    COL = fsSettings["image_width"];

    // 调用对象方法posegraph.loadVocabulary()，加载Brief字典，设置字典database对象（db）
    std::string pkg_path = ros::package::getPath("pose_graph"); // pose_graph所在Package的路径
    string vocabulary_file = pkg_path + "/../support_files/brief_k10L6.bin";  // Brief字典文件的路径
    ...
    // 读取yaml文件中的其他参数
    m_camera = camodocal::CameraFactory::instance()->generateCameraFromYamlFile(config_file.c_str()); // 读取相机模型参数
    fsSettings["image_topic"] >> IMAGE_TOPIC; // 图像话题
    fsSettings["pose_graph_save_path"] >> POSE_GRAPH_SAVE_PATH; // 位姿图保存路劲
    fsSettings["output_path"] >> VINS_RESULT_PATH;  // VINS结果输出路劲

    fsSettings["save_image"] >> DEBUG_IMAGE;                            // 是否输出位姿图的图片，用于调试: 1
    VISUALIZE_IMU_FORWARD = fsSettings["visualize_imu_forward"];        // 是否输出IMU前向传播的高频结果: 0
    LOAD_PREVIOUS_POSE_GRAPH = fsSettings["load_previous_pose_graph"];  // 是否从'pose_graph_save_path'加载之前保存的位姿图: 0
    FAST_RELOCALIZATION = fsSettings["fast_relocalization"];            // 是否打开快速重定位: 0

    VINS_RESULT_PATH = VINS_RESULT_PATH + "/vins_result_loop.csv";  // VINS结果输出路劲+文件名
    std::ofstream fout(VINS_RESULT_PATH, std::ios::out);
    fout.close();
    fsSettings.release();

    // 如果LOAD_PREVIOUS_POSE_GRAPH=1，则调用对象方法posegraph.loadPoseGraph()，加载先前的位姿图，令加载标志load_flag = 1
  ...
  // 6. 声明位姿图主线程对象并指定其线程函数: std::thread measurement_process = std::thread(process);
  // 7. 声明键盘操作线程对象并指定其线程函数: std::thread keyboard_command_process = std::thread(command);
